...
import static com.lreas.grpc.GrpcQuizServiceOuterClass.CreateQuizGrpc;
import static com.lreas.grpc.GrpcQuizServiceOuterClass.Collaborator;
import static com.lreas.grpc.GrpcQuizServiceOuterClass.ChoiceGrpc;
import static com.lreas.grpc.GrpcQuizServiceOuterClass.QuestionGrpc;
import static com.lreas.grpc.GrpcQuizServiceOuterClass.QuizResourcesGrpc;
...
@GrpcService
public class GrpcQuizServiceImpl extends GrpcQuizServiceGrpc.GrpcQuizServiceImplBase {
    private static final Logger logger = LoggerFactory.getLogger(GrpcQuizServiceImpl.class.getName());

    private final QuizService quizService;

    @Autowired
    public GrpcQuizServiceImpl(
    ...
    public void createQuiz(
            CreateQuizGrpc request,
            StreamObserver<QuizResourcesGrpc> responseObserver
    ) {
        try {
            CreateQuizDto createQuizDto = new CreateQuizDto();
            createQuizDto.contentName = request.getContentName().isEmpty() ? null : request.getContentName();

            createQuizDto.collaborators = new LinkedList<>();
            for (Collaborator collab : request.getCollaboratorsList()) {
                CreateQuizDto.Collaborator collaborator = new CreateQuizDto.Collaborator();
                collaborator.id = collab.getId();
                collaborator.role = ResourceAccessedBy.ROLE.valueOf(collab.getRole());

                createQuizDto.collaborators.add(collaborator);
            }

            createQuizDto.parentResourceId = request.getParentResourceId().isEmpty() ? null : request.getParentResourceId();
            createQuizDto.userId = request.getUserId().isEmpty() ? null : request.getUserId();
            ...
            // return
            responseObserver.onNext(quizResourcesGrpc);
            responseObserver.onCompleted();
        }
        catch (Exception ex) {
            logger.error("Grpc Create Quiz: {}", ex.getMessage());
    ...
    public void updateQuiz(
            QuizResourcesGrpc request,
            StreamObserver<Empty> responseObserver
    ) {
        try {
            // convert data
            QuizResourcesDto quizResourcesDto = this.convertToQuizResourcesDto(request);

            // pass to service
            this.quizService.updateQuiz(quizResourcesDto);
            ...
            logger.error("Grpc Update Quiz: {}", ex.getMessage());
            responseObserver.onError(ex);
        }
    }
    ...
    private QuizResourcesGrpc convertToQuizResourcesGrpc(
        ...
        for (QuestionDto question : quizResourcesDto.questions) {
            ...
            for (AbstractChoiceDto choice : question.choices) {
